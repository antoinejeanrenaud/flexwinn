...
Here we have a metabolite that is not distorted, so the function changomics is not really correcting it. The algorithm has found no change point which is correct.
...
```
The plates are displayed, the correction applied is displayed. The time series displayed here is different than with the changomics function because there is an additional step before detrending.
